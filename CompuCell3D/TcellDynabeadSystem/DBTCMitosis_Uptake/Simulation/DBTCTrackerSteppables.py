...
class TrajectoryTrackerSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        with PDBWriter(r"C:\Users\norac\OneDrive - UAB\Escritorio\uab\5\TFGJordi\ExperimentalData\TcellsDynabeadSystem\DBTCMitosis_Uptake\DBTC_InitialFrame.pdb") as pdb_writer:
            pdb_writer.write(self.universe.atoms)

        self.trajectory_writer = XTCWriter(r"C:\Users\norac\OneDrive - UAB\Escritorio\uab\5\TFGJordi\ExperimentalData\TcellsDynabeadSystem\DBTCMitosis_Uptake\DBTC_trajectories.xtc", self.num_atoms)
...
class NutrientFieldSteppable(SteppableBasePy):
    def __init__(self, frequency=1):
        ...
        self.activated_uptake_multiplier = 3.0  # Activated T cells consume more nutrients
        
    def start(self):
        self.nutrient_field = self.field.Nutrient
        
        # Create a file to track average nutrient levels
        self.nutrient_level_file = open(r"C:\Users\norac\OneDrive - UAB\Escritorio\uab\5\TFGJordi\ExperimentalData\TcellsDynabeadSystem\DBTCMitosis_Uptake\nutrient_levels.txt", "w")
...
class TcellGrowthSteppable(MitosisSteppableBase):
    def __init__(self, frequency = 1):
        ...
        self.nutrient_consumption_threshold = 0.05  # Minimum nutrients needed for growth
        self.max_growth_nutrients = 0.5  # Nutrients needed for maximum growth rate
        self.base_growth_rate = 1.0 / 150.0  # Base growth rate (pixels/MCS) - slower than original
        self.max_growth_rate = 1.0 / 80.0  # Maximum growth rate with optimal nutrients
...
class TCellMitosisSteppable(MitosisSteppableBase):
    def __init__(self,frequency=1):
        ...
        self.nutrient_division_threshold = 0.3  # Minimum recent nutrient consumption for division


    def start(self):
        self.TC_count_file = open(r"C:\Users\norac\OneDrive - UAB\Escritorio\uab\5\TFGJordi\ExperimentalData\TcellsDynabeadSystem\DBTCMitosis_Uptake\TC_count.txt", "w")
...
class TCellActivationSteppable(SteppableBasePy):
    ...
    def start(self):
        self.activation_count_file = open(r"C:\Users\norac\OneDrive - UAB\Escritorio\uab\5\TFGJordi\ExperimentalData\TcellsDynabeadSystem\DBTCMitosis_Uptake\activated_cells.txt", "w")
        self.activation_count_file.write("MCS\tNormalTCells\tActivatedTCells\n")

    def step(self, mcs):
        # Check ALL T cells (both regular and currently activated)
        # and update their activation state based on dynabead contact
        for cell in self.cell_list_by_type(self.TCELL, self.ACTIVATEDTCELL):
            is_touching = self.is_touching_dynabead(cell)
            
            # If in contact with dynabead and not activated, activate it
            if is_touching and cell.type == self.TCELL:
            ...
            # If not in contact with dynabead but activated, deactivate it
            elif not is_touching and cell.type == self.ACTIVATEDTCELL:
                # Preserve cell properties when changing type
                original_target_volume = cell.targetVolume
                original_target_surface = cell.targetSurface
                original_lambda_volume = cell.lambdaVolume
