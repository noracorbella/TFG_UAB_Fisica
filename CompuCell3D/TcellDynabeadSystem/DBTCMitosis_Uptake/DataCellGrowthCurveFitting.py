...
cell_count_file = r"/mnt/c/Users/norac/OneDrive - UAB/Escritorio/uab/5/TFGJordi/ExperimentalData/TcellsDynabeadSystem/DBTCMitosis_Uptake/DBTCMitosis_Uptake_19/TC_count.txt"
cell_count_png = r"/mnt/c/Users/norac/OneDrive - UAB/Escritorio/uab/5/TFGJordi/ExperimentalData/TcellsDynabeadSystem/DBTCMitosis_Uptake/TC_count_plot_19.png"
    ...
    try:
        ...
        try:
            if name != 'Logistic Growth':  # Logistic growth needs bounds
                ...
                print(f"{name} (unbounded) - R² = {r2:.4f}")
...
if best_model is not None:
    ...
    # Plot original data with smaller points and transparency
    ...
    # Calculate some biological parameters if using Gompertz or Logistic
    if 'Gompertz' in best_name or 'Logistic' in best_name:
        print(f"\nBiological interpretation:")
        if 'Gompertz' in best_name:
            print(f"Carrying capacity ≈ {best_params[0] + best_params[3]:.2f}")
            print(f"Initial population ≈ {best_params[3]:.2f}")
        elif 'Logistic' in best_name:
            print(f"Carrying capacity (K) = {best_params[0]:.2f}")
            print(f"Growth rate (r) = {best_params[1]:.2e}")
            print(f"Initial population (N0) = {best_params[3]:.2f}")
            
else:
    print("No suitable model found. Your data might not follow standard growth patterns.")
    print("Consider checking your data or using a different modeling approach.")
    
    # Still plot the data for visualization
    plt.figure(figsize=(10, 6))
