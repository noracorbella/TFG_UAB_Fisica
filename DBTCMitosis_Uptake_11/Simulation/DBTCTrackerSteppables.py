...
class TrajectoryTrackerSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        with PDBWriter(r"C:\Users\norac\OneDrive - UAB\Escritorio\uab\5\TFGJordi\ExperimentalData\TcellsDynabeadSystem\DBTCMitosis_Uptake\DBTCMitosis_Uptake_11\DBTC_frame0.pdb") as pdb_writer:
            pdb_writer.write(self.universe.atoms)

        self.trajectory_writer = XTCWriter(r"C:\Users\norac\OneDrive - UAB\Escritorio\uab\5\TFGJordi\ExperimentalData\TcellsDynabeadSystem\DBTCMitosis_Uptake\DBTCMitosis_Uptake_11\DBTC_traj.xtc", self.num_atoms)
...
class NutrientFieldSteppable(SteppableBasePy):
    def __init__(self, frequency=1):
        ...
        self.base_uptake_rate = 0.001  # Base uptake rate for regular T cells
        self.activated_uptake_multiplier = 1.0  # Activated T cells consume more nutrients

        self.max_nutrient_concentration = 10.0  # Maximum nutrient concentration
        
        self.minimum_nutrient_threshold = 0.2  #Minimum nutrient concentration to maintain
    ...
    def start(self):
        ...
        self.nutrient_uptake_file = open(r"C:\Users\norac\OneDrive - UAB\Escritorio\uab\5\TFGJordi\ExperimentalData\TcellsDynabeadSystem\DBTCMitosis_Uptake\DBTCMitosis_Uptake_11\nutrient_uptake_data.txt", "w")
    ...
    def step(self, mcs):
        ...
        for cell in self.cell_list:
            if cell.type == self.TCELL or cell.type == self.ACTIVATEDTCELL:
                ...
                # Make uptake proportional to concentration
                # At max concentration, cells consume at full rate
                # At lower concentrations, uptake scales proportionally
                concentration_factor = current_conc / self.max_nutrient_concentration
                proportional_uptake_rate = uptake_rate * concentration_factor

                
                
                # Check if we're above the minimum threshold
                if current_conc > self.minimum_nutrient_threshold:
                    # Calculate how much we can consume without going below threshold
                    max_allowable_uptake = current_conc - self.minimum_nutrient_threshold
                    # Actual uptake is the minimum of desired uptake and allowable uptake
                    actual_uptake = min(proportional_uptake_rate, max_allowable_uptake)
                    
                    # Update nutrient field, ensuring it doesn't go below threshold
                    self.nutrient_field[x, y, 0] = current_conc - actual_uptake


                else:
                    # At minimum threshold - allow consumption but maintain minimum
                    # Scale the uptake based on the minimum threshold
                    min_concentration_factor = self.minimum_nutrient_threshold / self.max_nutrient_concentration
                    actual_uptake = uptake_rate * min_concentration_factor
                    # Nutrient field stays at minimum
...
class TcellGrowthSteppable(MitosisSteppableBase):
    def __init__(self, frequency = 1):
        ...
        self.nutrient_consumption_threshold = 0.0  # Minimum nutrients needed for growth
        self.max_growth_nutrients = 0.05  # Nutrients needed for maximum growth rate
        self.base_growth_rate = 0.005  # Base growth rate (pixels/MCS) - slower than original
        self.max_growth_rate = 0.05  # Maximum growth rate with optimal nutrients
                ...
                if cell.volume <= max_volume:
                    ...
                    if current_pressure < self.pressure_threshold:    
                        ...
                        if nutrients_consumed >= self.nutrient_consumption_threshold:
                            ...
                            uptake_factor = min(1.0, nutrients_consumed / self.max_growth_nutrients)
                            
                            # Calculate growth rate interpolating between base and max
                            growth_rate = self.base_growth_rate + (self.max_growth_rate - self.base_growth_rate) * uptake_factor
...
class TCellMitosisSteppable(MitosisSteppableBase):
    def __init__(self,frequency=1):
        ...
        self.nutrient_division_threshold = 0.0  # Minimum recent nutrient consumption for division
        self.max_divisions = 5

    def start(self):
        self.TC_count_file = open(r"C:\Users\norac\OneDrive - UAB\Escritorio\uab\5\TFGJordi\ExperimentalData\TcellsDynabeadSystem\DBTCMitosis_Uptake\DBTCMitosis_Uptake_11\TC_count.txt", "w")
...
class EnergyTrackerSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        self.output_file = open(r"C:\Users\norac\OneDrive - UAB\Escritorio\uab\5\TFGJordi\ExperimentalData\TcellsDynabeadSystem\DBTCMitosis_Uptake\DBTCMitosis_Uptake_11\energy_file.txt", "w")
        ...
        self.data_file = open(r"C:\Users\norac\OneDrive - UAB\Escritorio\uab\5\TFGJordi\ExperimentalData\TcellsDynabeadSystem\DBTCMitosis_Uptake\DBTCMitosis_Uptake_11\cell_data.txt", "w")
